...
// Kahoot-like home screen with three primary actions: Play Solo, Create Group, Join Group
...
class _QuizHomeState extends State<QuizHome> with SingleTickerProviderStateMixin {
  ...
  // Guest user placeholders
  String? get _userDisplayName => null;
  String? get _userEmail => null;
  String? get _userPhotoUrl => null;
  ...
  Future<void> _signOut() async {
    // No-op while authentication is disabled.
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Sign out disabled (auth disabled).')),
    );
  }

  // Quiz start flow is handled by Play Solo / Group flows.
      ...
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [
                  cs.primary.withAlpha((0.90 * 255).round()), // compat across SDKs
                      ...
                      : null,
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(
                _userDisplayName?.trim().isNotEmpty == true
                  ? _userDisplayName!
                  : (_userEmail ?? 'Guest'),
                                            style: TextStyle(
                                              fontSize: 16,
